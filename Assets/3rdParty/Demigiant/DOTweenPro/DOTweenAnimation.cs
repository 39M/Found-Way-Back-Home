...
namespace DG.Tweening
{
    ...
    public class DOTweenAnimation : ABSAnimationComponent
    {
        ...
        public bool useStartTargetAsV3;
        ...
        [NonSerialized]
        public bool realValueCalced = false;
        [NonSerialized]
        public Vector3 realStartValueV3;
        [NonSerialized]
        public Vector3 realEndValueV3;
        ...
        public bool tweenCreated; // TRUE after the tweens have been created
        int _playCount = -1; // Used when calling DOPlayNext

        //private ContentSizeFitter csf;
        [HideInInspector] public bool toggleDirection = true;
        ...
        public void CreateTween()
        {
            ...
            switch (animationType)
            {
                ...
                case DOTweenAnimationType.LocalMove:
                    ...
                    {
                        // 对于 RectTransfrom 其实想要的效果是 DOAnchorPos()，
                        // 并且 start、end 都是相对于现在的位置的：因为刘海屏适配会推UI，所以不能填绝对位置

                        if (!realValueCalced)
                        {
                            realValueCalced = true;
                            var originalAnchoredPosition = rectTransform.anchoredPosition3D;
                            realStartValueV3 = originalAnchoredPosition + startValueV3;
                            realEndValueV3 = originalAnchoredPosition + endValueV3;
                        }

                        rectTransform.anchoredPosition3D = realStartValueV3;
                        tween = rectTransform.DOAnchorPos3D(realEndValueV3, duration, optionalBool0);
                    }
                    else
                    {
            ...
            tween.SetTarget(this.gameObject).SetDelay(delay).SetLoops(loops, loopType).SetAutoKill(autoKill).OnKill(() => tween = null);
            if (isSpeedBased)
            {
                tween.SetSpeedBased();
            }
        ...
        void ReEvaluateRelativeTween()
        {
            ...
            else if (animationType == DOTweenAnimationType.LocalMove)
            {
                ...
                {
                    ((Tweener)tween).ChangeEndValue(realEndValueV3, true);
                }
            }
        }
        ...
        public void Reset()
        {
            switch (animationType)
            {
                ...
                case DOTweenAnimationType.Move:
                    if (useTargetAsV3)
                    {
                        if (startValueTransform != null)
                        {
                            target.transform.position = startValueTransform.position;
                            //if (targetType == TargetType.RectTransform)
                            //{
                            //    RectTransform startValueT = startValueTransform as RectTransform;
                            //    if (startValueT != null)
                            //    {
                            //        RectTransform rTarget = target as RectTransform;
                            //        if (rTarget != null)
                            //        {
                            //            startValueV3 = DOTweenUtils46.SwitchToRectTransform(startValueT, rTarget);
                            //        }
                            //    }
                            //}
                        }
                    ...
                    {
                        target.transform.position = startValueV3;
                    }
                    //csf = target.gameObject.GetComponent<ContentSizeFitter>();
                ...
                case DOTweenAnimationType.LocalMove:
                    if (rectTransform == null)
                    {
                        target.transform.localPosition = startValueV3;
                    }
                    else
                    {
                        rectTransform.anchoredPosition3D = realStartValueV3;
                    ...
                    break;
                case DOTweenAnimationType.Rotate:
                    target.transform.rotation = Quaternion.Euler(startValueV3);
                    break;
                case DOTweenAnimationType.LocalRotate:
                    target.transform.localRotation = Quaternion.Euler(startValueV3);
                    break;
                case DOTweenAnimationType.Scale:
                    target.transform.localScale = optionalBool0 ? new Vector3(startValueFloat, startValueFloat, startValueFloat) : startValueV3;
                    break;
                case DOTweenAnimationType.UIWidthHeight:
                    RectTransform tar = target as RectTransform;
                    tar.sizeDelta = optionalBool0 ? new Vector2(startValueFloat, startValueFloat) : startValueV2;
                    break;
                case DOTweenAnimationType.Color:
                    switch (targetType)
                    {
                        case TargetType.SpriteRenderer:
                            SpriteRenderer t1 = target as SpriteRenderer;
                            t1.color = startValueColor;
                            break;
                        case TargetType.Renderer:
                            Renderer t2 = target as Renderer;
                            t2.material.color = startValueColor;
                            break;
                        case TargetType.Image:
                            Image t3 = target as Image;
                            t3.color = startValueColor;
                            break;
                        case TargetType.Text:
                            Text t4 = target as Text;
                            t4.color = startValueColor;
                            break;
                        case TargetType.Light:
                            Light t5 = target as Light;
                            t5.color = startValueColor;
                            break;
                        //case TargetType.TextMeshProUGUI:
                        //    TextMeshProUGUI t8 = target as TextMeshProUGUI;
                        //    t8.color = startValueColor;
                        //    break;
                        //case TargetType.TextMeshPro:
                        //    TextMeshPro t9 = target as TextMeshPro;
                        //    t9.color = startValueColor;
                        //    break;
                    }
                    break;
                case DOTweenAnimationType.Fade:
                    switch (targetType)
                    {
                        case TargetType.SpriteRenderer:
                            SpriteRenderer t1 = target as SpriteRenderer;
                            t1.color = ChangeAlpha(t1.color, startValueFloat);
                            break;
                        case TargetType.Renderer:
                            Renderer t2 = target as Renderer;
                            t2.material.color = ChangeAlpha(t2.material.color, startValueFloat);
                            break;
                        case TargetType.Image:
                            Image t3 = target as Image;
                            t3.color = ChangeAlpha(t3.color, startValueFloat);
                            break;
                        case TargetType.Text:
                            Text t4 = target as Text;
                            t4.color = ChangeAlpha(t4.color, startValueFloat);
                            break;
                        case TargetType.Light:
                            Light t5 = target as Light;
                            t5.color = ChangeAlpha(t5.color, startValueFloat);
                            break;
                        case TargetType.CanvasGroup:
                            CanvasGroup t6 = target as CanvasGroup;
                            t6.alpha = startValueFloat;
                            break;
                        case TargetType.TextMeshProUGUI:
                            break;
                        case TargetType.TextMeshPro:
                ...
                case DOTweenAnimationType.PunchPosition:
                    ...
                    break;
                case DOTweenAnimationType.PunchScale:
                    target.transform.localScale = startValueV3;
                    break;
                case DOTweenAnimationType.PunchRotation:
                    target.transform.localRotation = Quaternion.Euler(startValueV3);
                    break;
                ...
                case DOTweenAnimationType.CameraAspect:
                    ...
                    t.aspect = startValueFloat;
                    break;
                case DOTweenAnimationType.CameraBackgroundColor:
                    t = target as Camera;
                    t.backgroundColor = startValueColor;
                    break;
                case DOTweenAnimationType.CameraFieldOfView:
                    t = target as Camera;
                    t.fieldOfView = startValueFloat;
                    break;
                case DOTweenAnimationType.CameraOrthoSize:
                    t = target as Camera;
                    t.orthographicSize = startValueFloat;
                    break;
                case DOTweenAnimationType.CameraPixelRect:
                    t = target as Camera;
                    t.pixelRect = startValueRect;
                    break;
                case DOTweenAnimationType.CameraRect:
                    t = target as Camera;
                    t.rect = startValueRect;
                    break;
            }
        }
    }
