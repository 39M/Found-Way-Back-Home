...
namespace MagicaCloth
{
    ...
    public class PhysicsManagerBoneData : PhysicsManagerAccess
    {
        ...
        /// 復元ボーンフラグビット
        /// </summary>
        public const byte RestoreBone_FixedUpdate = 0x01;     // FixedUpdateタイミングで復元する
        ...
        /// <param name="fixedUpdate">true=FixedUpdateで復元する</param>
        /// <returns></returns>
        public int AddRestoreBone(Transform target, float3 lpos, quaternion lrot, bool fixedUpdate)
        {
            ...
            else
            {
                ...
                restoreBoneFlagList.Add(0);
                hasBoneChanged = true;
            }

            // flag
            SetRestoreBoneFlag(restoreBoneIndex, RestoreBone_FixedUpdate, fixedUpdate);
        ...
        /// <param name="sw"></param>
        public void SetRestoreBoneFlag(int restoreBoneIndex, byte flag, bool sw)
        {
            byte data = restoreBoneFlagList[restoreBoneIndex];
            if (sw)
            {
                data |= flag;
            }
            ...
            restoreBoneFlagList[restoreBoneIndex] = data;
        }
        ...
        public void ResetBoneFromTransform(bool fixedUpdate)
        {
            ...
            if (RestoreBoneCount > 0)
            {
                var job = new RestoreBoneJob()
                {
                    ...
                    restoreBoneFlagList = restoreBoneFlagList.ToJobArray(),
                };
        ...
        struct RestoreBoneJob : IJobParallelForTransform
        {
            ...
            [Unity.Collections.ReadOnly]
            public NativeArray<byte> restoreBoneFlagList;

            // 復元ボーンごと
            public void Execute(int index, TransformAccess transform)
            {
                bool sw = (restoreBoneFlagList[index] & RestoreBone_FixedUpdate) != 0;
                if (sw == fixedUpdate)
                {
