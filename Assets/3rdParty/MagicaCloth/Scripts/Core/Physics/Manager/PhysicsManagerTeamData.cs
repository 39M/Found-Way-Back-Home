    ...
    public class PhysicsManagerTeamData : PhysicsManagerAccess
    {
        ...
        public void SetUpdateMode(int teamId, PhysicsTeam.TeamUpdateMode updateMode)
        {
            ...
            for (int i = 0; i < chunk.dataLength; i++)
            {
                ...
                if (flag.IsValid() && flag.IsRestoreTransform())
                {
                    // particle flag
                    flag.SetFlag(PhysicsManagerParticleData.Flag_Transform_Restore_FixedUpdate, fixedUpdate);
                    Particle.flagList[pindex] = flag;

                    // bone flag
                    int restoreBoneIndex = Particle.restoreTransformIndexList[pindex];
                    if (restoreBoneIndex >= 0)
                    {
                        Bone.SetRestoreBoneFlag(restoreBoneIndex, PhysicsManagerBoneData.RestoreBone_FixedUpdate, fixedUpdate);
                    }
                }
            }
        }
        ...
        public int CalcMaxUpdateCount(int ups, float deltaTime, float physicsDeltaTime, float updateDeltaTime)
        {
            ...
            foreach (var kv in teamComponentDict)
            {
                ...
                {
                    // 固定更新は１回（ただしLateUpdate更新は除く）
                    cnt = 1;
                }
                else
                {
                    float timeScale = tdata.timeScale * globalTimeScale;
                    //float addTime = deltaTime * timeScale;
