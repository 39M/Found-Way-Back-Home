            ...
            public void Execute(int index)
            {
                ...
                for (int i = 0; i < 2; i++)
                {
                    ...
                    for (int j = 0; j < c.useLength; j++, dataIndex++)
                    {
                        ...
                        {
                            // カプセルコライダー判定
                            dist = CapsuleColliderDetection(ref nextpos, basepos, radius, cindex, new float3(1, 0, 0));
                        }
                        else if (cflag.IsFlag(PhysicsManagerParticleData.Flag_CapsuleY))
                        {
                            // カプセルコライダー判定
                            dist = CapsuleColliderDetection(ref nextpos, basepos, radius, cindex, new float3(0, 1, 0));
                        }
                        else if (cflag.IsFlag(PhysicsManagerParticleData.Flag_CapsuleZ))
                        {
                            // カプセルコライダー判定
                            dist = CapsuleColliderDetection(ref nextpos, basepos, radius, cindex, new float3(0, 0, 1));
                        }
                        ...
                        {
                            // 球コライダー判定
                            dist = SphereColliderDetection(ref nextpos, basepos, radius, cindex);
                        }
